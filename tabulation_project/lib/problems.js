...
//   // start at nums[i], track index
//   // keep going until nums[i] === undefined
//   // return num[i]
...
function stepper(nums, memo = nums[0]) {
  for (let i = 1; i <= memo; i++) {
    if (nums[i] >= memo) {
      stepper(nums.slice(i), (memo = nums[i]));
...
function maxNonAdjacentSum(nums) {
  ...
  return table[0] > table[1] ? table[0] : table[1];
}
...
function minChange(coins, amount) {
  ...
  let table = new Array(...coins); // creates new array
